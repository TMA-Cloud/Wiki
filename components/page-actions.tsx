...
// `usePathname()` and static asset routes are both relative to the deployment
// root, so every URL we hand out has to carry the GitHub Pages base path.
...
export function ViewOptionsPopover({
  ...
  githubUrl,
  ...props
}: Omit<ComponentProps<typeof PopoverTrigger>, 'className'> & {
  markdownUrl?: string;
  githubUrl?: string;
  className?: string;
}) {
  ...
  const items = useMemo(() => {
    // Always point AI tools at the public canonical URL — a localhost address
    // or a base-path-less one is useless to them. `pathname` never includes the
    // base path, and `siteUrl` already ends with it.
    const pageUrl = siteUrl.replace(/\/$/, '') + pathname;
    ...
    ].filter((v) => !!v) as { title: string; href: string; icon: ReactNode }[];
