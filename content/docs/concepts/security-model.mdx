...
- **Segmented:** Objects are split into 1 MB segments, each authenticated on its own, with a per-file key derived from the master key via HKDF-SHA256. This lets downloads serve HTTP `Range` requests without reading the whole file
